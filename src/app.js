...
function getSyncStatus(synced) {
    return synced ? 'concluido' : 'pendente';
}

function prepararRegistroLocal(registro, synced = false) {
    return {
        ...registro,
        synced,
        sync_status: getSyncStatus(synced)
    };
}

function prepararPayloadServidor(registro) {
    const { synced, sync_status, editId, _deleted, ...payload } = registro;
    return payload;
}
...
function updateSyncStatus(storeName, id, synced) {
    return new Promise((resolve, reject) => {
        ...
        getRequest.onsuccess = () => {
            ...
            if (data) {
                data.synced = synced;
                data.sync_status = getSyncStatus(synced);
...
async function getLocalAquisicoes() {
    try {
        await ensureDBReady();
        return await getAllFromDB(STORE_AQUISICOES);
    } catch (error) {
        console.error('Erro ao ler aquisicoes locais:', error);
        return [];
    }
}

function mergeAquisicoesMobile(serverRecords = [], localRecords = []) {
    const merged = new Map();

    serverRecords
        .filter(aq => aq && aq.id)
        .forEach(aq => merged.set(aq.id, prepararRegistroLocal(aq, true)));

    localRecords
        .filter(aq => aq && aq.id && !aq._deleted)
        .forEach(aq => {
            const shouldPreferLocal = aq.synced === false || !merged.has(aq.id);
...
async function cacheServerAquisicoes(serverRecords = [], localRecords = []) {
    if (!db || !serverRecords.length) return;

    const localById = new Map(localRecords.map(aq => [aq.id, aq]));
    for (const aq of serverRecords) {
        if (!aq?.id) continue;
        const local = localById.get(aq.id);
        if (local?.synced === false || local?._deleted) continue;
        try {
            await saveToDB(STORE_AQUISICOES, prepararRegistroLocal(aq, true));
        } catch (error) {
            console.error('Erro ao cachear aquisicao:', error);
        }
    }
}

function upsertAquisicaoNaLista(aquisicao) {
    aquisicoesMobile = aquisicoesMobile.filter(aq => aq.id !== aquisicao.id);
    if (!aquisicao._deleted) {
        aquisicoesMobile.unshift(aquisicao);
    }
    ...
    try {
        ...
        for (const aq of unsyncedAq) {
            try {
                ...
                if (aq._deleted) {
                    await deleteFromDB(STORE_AQUISICOES, aq.id);
                } else {
...
async function syncAquisicaoToServer(aq) {
    ...
    const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(prepararPayloadServidor(aq)),
        signal: AbortSignal.timeout(15000)
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return await res.json();
...
async function fetchWithOfflineSupport(url, options = {}) {
    ...
    try {
        ...
        isOnline = false;
        atualizarStatusConexao('offline', 'Sem conexao com o servidor');
        return { offline: true, ok: false };
    }
}

function addToOfflineQueue(item) {
    console.warn('Fila offline generica nao implementada para esta acao:', item);
}
...
async function updateOfflineQueueIndicator() {
    ...
    document.querySelectorAll('#offline-queue-status').forEach(indicator => {
        const count = indicator.querySelector('#offline-queue-count');
        if (total > 0) {
...
async function carregarAquisicoesMobile() {
    ...
    const locais = await getLocalAquisicoes();
    if (locais.length) {
        aquisicoesMobile = mergeAquisicoesMobile([], locais);
        renderAquisicoesMobile();
    }
    ...
    try {
        ...
        aquisicoesMobile = mergeAquisicoesMobile(servidor, locais);
        await cacheServerAquisicoes(servidor, locais);
        renderAquisicoesMobile();
    } catch (err) {
        console.error(err);
        if (locais.length) {
            renderAquisicoesMobile();
            if (count) {
                count.textContent = `${aquisicoesMobile.length} local${aquisicoesMobile.length === 1 ? '' : 'is'} pendente${aquisicoesMobile.length === 1 ? '' : 's'}`;
            }
            return;
        }
    ...
    list.innerHTML = ordenadas.map(a => `
        <button class="aq-list-item" type="button" data-id="${a.id}">
            <div class="aq-list-head">
                ...
                <span class="status-pill ${a.synced === false ? 'pending-sync' : (a.status || 'em-execucao')}">${a.synced === false ? 'Pendente' : statusAquisicaoLabel(a.status)}</span>
            </div>
            <div class="aq-list-meta">
                ...
                ${a.synced === false ? '<span>Sincronizacao pendente</span>' : ''}
...
async function excluirAquisicaoMobileSelecionada() {
    ...
    try {
        ...
        if (result.offline) {
            await saveToDB(STORE_AQUISICOES, prepararRegistroLocal({
                ...aq,
                _deleted: true,
                deletadoEm: new Date().toISOString(),
                origem: aq.origem || 'mobile-app'
            }, false));
            aquisicoesMobile = aquisicoesMobile.filter(item => item.id !== aq.id);
            renderAquisicoesMobile();
            await updateOfflineQueueIndicator();
            fecharMenuAquisicaoMobile();
...
async function salvarNaoRealizadoMobile() {
    ...
    const atualizada = prepararRegistroLocal({
        ...aq,
    ...
    try {
        await saveToDB(STORE_AQUISICOES, atualizada);
        upsertAquisicaoNaLista(atualizada);
        await updateOfflineQueueIndicator();
        ...
        await updateSyncStatus(STORE_AQUISICOES, aq.id, true);
        atualizada.synced = true;
        atualizada.sync_status = getSyncStatus(true);
        upsertAquisicaoNaLista(atualizada);
        fecharMotivoNaoRealizadoMobile();
...
document.getElementById('form-aquisicao')?.addEventListener('submit', async (e) => {
    ...
    const novaAq = prepararRegistroLocal({
        id,
    ...
    try {
        ...
        if (isOnline) {
            try {
                ...
                const res = await fetch(url, {
                    method,
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(prepararPayloadServidor(novaAq)),
                    signal: AbortSignal.timeout(15000)
                });
                ...
                if (res.ok) {
                    ...
                    novaAq.synced = true;
                    novaAq.sync_status = getSyncStatus(true);
                    upsertAquisicaoNaLista(novaAq);
                    await updateOfflineQueueIndicator();
                } else {
        ...
        console.error(err);
        showToast('Erro ao salvar no armazenamento local do aparelho.');
    } finally {
        btn.disabled = false;
        btn.innerHTML = `
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
