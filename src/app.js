...
const OFFLINE_QUEUE_KEY = 'tf_offline_queue';
let serverUrl = '';
...
function addToOfflineQueue(operation) {
    ...
    showToast('Dados salvos offline. Serão sincronizados quando a conexão retornar.');
}
...
async function syncOfflineQueue() {
    if (syncInProgress || !isOnline || !serverUrl) return;
    
    const queue = getOfflineQueue();
    if (queue.length === 0) return;

    syncInProgress = true;
    showToast(`Sincronizando ${queue.length} dados offline...`);
    ...
    if (failedOperations.length === 0) {
        ...
        carregarAquisicoesMobile();
        loadAllCalendarData();
    } else if (failedOperations.length < queue.length) {
        showToast(`${queue.length - failedOperations.length} de ${queue.length} dados sincronizados.`);
    } else {
        showToast('Não foi possível sincronizar os dados. Tentaremos novamente.');
    }
}
...
async function fetchWithOfflineSupport(url, options = {}, skipQueue = false) {
    if (!isOnline && !skipQueue) {
        // Se estiver offline e não for uma operação de leitura, adicionar à fila
        ...
        // Se for uma operação de leitura offline, retornar dados locais se disponível
        return { offline: true };
    }
    ...
    try {
        ...
        if (!isOnline && options.method && options.method !== 'GET' && !skipQueue) {
            addToOfflineQueue({
                method: options.method,
                url: url,
                body: options.body ? JSON.parse(options.body) : null,
...
async function enviarOS(e) {
    ...
    try {
        // Enviar diretamente para o endpoint dedicado do app com suporte offline
        const result = await fetchWithOfflineSupport(`${serverUrl}/api/os`, {
            method: 'POST',
        ...
        if (!result.ok) throw new Error('Falha ao salvar a O.S.');
...
document.getElementById('form-aquisicao')?.addEventListener('submit', async (e) => {
    ...
    try {
        const result = await fetchWithOfflineSupport(editId ? `${serverUrl}/api/aquisicao/${encodeURIComponent(editId)}` : `${serverUrl}/api/aquisicao`, {
        ...
        if (result.offline) {
            ...
            document.getElementById('success-details-aq').innerHTML = `
                ...
                <div class="success-line" style="color: var(--warning);">
                    ...
                    <div><strong>Status:</strong> Salvo offline - Será sincronizado quando a conexão retornar</div>
                </div>
            `;
        ...
        if (!result.ok) throw new Error('Falha ao salvar a Aquisição.');
...
async function salvarEventoMobile(e) {
    ...
    try {
        ...
        if (getRes.offline) {
            // Salvar evento localmente para sincronização
            addToOfflineQueue({
                method: 'POST',
                url: `${serverUrl}/api/data`,
                body: { action: 'add_event', event: novo },
                headers: { 'Content-Type': 'application/json' }
            });
            document.getElementById('event-modal').classList.add('hidden');
            showToast('Evento salvo offline. Será sincronizado quando a conexão retornar.');
            loadAllCalendarData();
            return;
        }
