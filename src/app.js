...
function registrarEventos() {
    ...
    document.getElementById('btn-aq-view-close')?.addEventListener('click', fecharVisualizacaoAquisicaoMobile);
...
function visualizarAquisicaoMobileSelecionada() {
    ...
    content.innerHTML = `
        <div class="aq-view-detail">
            <strong>Nº Compra:</strong> ${aq.numeroCompra || '-'}
        </div>
        <div class="aq-view-detail">
            <strong>Item/Material:</strong> ${aq.item || '-'}
        </div>
        <div class="aq-view-detail">
            <strong>Data:</strong> ${formatarDataMobile(aq.data)}
        </div>
        <div class="aq-view-detail">
            <strong>Fornecedor:</strong> ${aq.fornecedor || '-'}
        </div>
        <div class="aq-view-detail">
            <strong>Veículo:</strong> ${aq.veiculo || '-'}
        </div>
        <div class="aq-view-detail">
            <strong>Quantidade:</strong> ${aq.quantidade || '-'}
        </div>
        <div class="aq-view-detail">
            <strong>Valor:</strong> ${aq.valor ? `R$ ${aq.valor.toFixed(2)}` : '-'}
        </div>
        <div class="aq-view-detail">
            <strong>Responsável:</strong> ${aq.responsavel || '-'}
        </div>
        ${aq.motoristaLogistica ? `
        <div class="aq-view-detail" style="color: var(--primary);">
            <strong>Motorista da Logística:</strong> Sim
        </div>
        ` : ''}
        <div class="aq-view-detail">
            <strong>Status:</strong> ${statusAquisicaoLabel(aq.status)}
        </div>
        ${aq.obs ? `
        <div class="aq-view-detail">
            <strong>Observações:</strong> ${aq.obs}
        </div>
        ` : ''}
        ${aq.status === 'nao-concluido' && aq.motivo ? `
        <div class="aq-view-detail" style="color: var(--danger);">
            <strong>Motivo não realizado:</strong> ${aq.motivo}
        </div>
        ` : ''}
        <div class="aq-view-detail" style="font-size: 12px; color: var(--text-secondary); margin-top: 16px;">
            <strong>Criado em:</strong> ${aq.criadoEm ? new Date(aq.criadoEm).toLocaleString('pt-BR') : '-'}
        </div>
    `;
    document.getElementById('aq-view-modal').classList.remove('hidden');
}

function fecharVisualizacaoAquisicaoMobile() {
    document.getElementById('aq-view-modal')?.classList.add('hidden');
}
...
function showDayEvents(dateStr, evs, oss, aqs) {
    ...
    titleEl.textContent = `Eventos de ${formattedDate}`;
    
    let content = '';
    if (evs.length > 0) {
        content += `<div style="margin-bottom: 12px;"><strong>Lembretes:</strong><br>`;
        evs.forEach(ev => {
            content += `<div style="padding: 4px 0; border-bottom: 1px solid var(--border);">${ev.titulo} (${ev.criticidade})</div>`;
        });
        content += `</div>`;
    }
    if (oss.length > 0) {
        content += `<div style="margin-bottom: 12px;"><strong>Ordens de Serviço:</strong><br>`;
        oss.forEach(os => {
            content += `<div style="padding: 4px 0; border-bottom: 1px solid var(--border);">OS: ${os.veiculo} - ${os.tipoServico}</div>`;
        });
        content += `</div>`;
    }
    if (aqs.length > 0) {
        content += `<div style="margin-bottom: 12px;"><strong>Aquisições:</strong><br>`;
        aqs.forEach(aq => {
            content += `<div style="padding: 4px 0; border-bottom: 1px solid var(--border);">Aq: ${aq.item} - ${aq.veiculo || 'Estoque'}</div>`;
        });
        content += `</div>`;
    }
    
    typeEl.innerHTML = content;
