...
function registrarEventos() {
    ...
    document.getElementById('btn-aq-view')?.addEventListener('click', verDetalhesAquisicaoMobile);
    ...
    document.getElementById('btn-aq-not-done')?.addEventListener('click', () => abrirMotivoNaoRealizadoMobile('nao-concluido'));
    document.getElementById('btn-aq-not-found')?.addEventListener('click', () => abrirMotivoNaoRealizadoMobile('nao-encontrado'));
    ...
    document.getElementById('btn-back-detalhes-aq')?.addEventListener('click', () => {
...
function preencherSelectsAquisicao() {
    ...
    if (selResp && typeof responsaveisBusca !== 'undefined') {
        selResp.innerHTML = '<option value="">Selecionar quem buscou *</option>' +
            responsaveisBusca.map(m => `<option value="${m}">${m}</option>`).join('');
    }
    // Data atual
...
function statusAquisicaoLabel(status) {
    return {
        ...
        'nao-encontrado': 'Não encontrado na rua'
    }[status] || 'Em execução';
}
...
async function carregarAquisicoesMobile() {
    ...
    try {
        ...
        aqPageMobile = 0; // Reset page on fresh load
...
function renderAquisicoesMobile() {
    ...
    if (!ordenadas.length) {
        ...
        const pag = document.getElementById('aq-paginacao-mobile');
        if (pag) pag.innerHTML = '';
        return;
    }

    const total = ordenadas.length;
    const totalPages = Math.max(1, Math.ceil(total / AQ_PER_PAGE_MOBILE));
    if (aqPageMobile >= totalPages) aqPageMobile = totalPages - 1;
    if (aqPageMobile < 0) aqPageMobile = 0;

    const paginated = ordenadas.slice(aqPageMobile * AQ_PER_PAGE_MOBILE, (aqPageMobile + 1) * AQ_PER_PAGE_MOBILE);
    ...
    list.innerHTML = paginated.map(a => `
        <button class="aq-list-item" type="button" data-id="${a.id}">
            ...
            <div class="aq-list-meta">
                ${a.numeroCompra ? `<span>Compra: ${a.numeroCompra}</span>` : ''}
                ${a.numeroNota ? `<span>Nota: ${a.numeroNota}</span>` : ''}
                ...
                ${(a.status === 'nao-concluido' || a.status === 'nao-encontrado') && a.motivo ? `<span>Motivo: ${a.motivo}</span>` : ''}
            </div>
        </button>
    `).join('');
    ...
    if (pag) {
        pag.innerHTML = `
            <button id="btn-aq-prev-mobile" style="padding:8px 16px; border-radius:8px; border:1px solid var(--border); background:white; color:var(--text); font-weight:600; cursor:pointer;" ${aqPageMobile === 0 ? 'disabled style="opacity:0.5; cursor:default;"' : ''}>&larr;</button>
            <span style="font-weight:600; font-size:14px; color:var(--text-secondary);">Pág. ${aqPageMobile + 1} de ${totalPages}</span>
            <button id="btn-aq-next-mobile" style="padding:8px 16px; border-radius:8px; border:1px solid var(--border); background:white; color:var(--text); font-weight:600; cursor:pointer;" ${aqPageMobile === totalPages - 1 ? 'disabled style="opacity:0.5; cursor:default;"' : ''}>&rarr;</button>
        `;

        document.getElementById('btn-aq-prev-mobile')?.addEventListener('click', () => {
            if (aqPageMobile > 0) {
                aqPageMobile--;
                renderAquisicoesMobile();
            }
        ...
        document.getElementById('btn-aq-next-mobile')?.addEventListener('click', () => {
            if (aqPageMobile < totalPages - 1) {
                aqPageMobile++;
                renderAquisicoesMobile();
            }
        });
...
function abrirMenuAquisicaoMobile(id) {
    ...
    let subtitleText = `${aq.fornecedor || '-'} · ${statusAquisicaoLabel(aq.status)}`;
    if (aq.numeroNota) {
        subtitleText += ` · Nota: ${aq.numeroNota}`;
    }
    document.getElementById('aq-action-subtitle').textContent = subtitleText;
...
function verDetalhesAquisicaoMobile() {
    const aq = getAquisicaoMobileSelecionada();
    if (!aq) return;
    fecharMenuAquisicaoMobile();

    const statusLabel = statusAquisicaoLabel(aq.status);
    const statusColor = aq.status === 'concluido' ? '#38a169' : (aq.status === 'em-execucao' ? '#d69e2e' : '#e53e3e');
    const dataFmt = aq.data ? aq.data.split('-').reverse().join('/') : '-';
    const valorFmt = aq.valor ? `R$ ${parseFloat(aq.valor).toFixed(2).replace('.', ',')}` : '-';

    const content = document.getElementById('aq-detalhes-content');
    content.innerHTML = `
        <div class="form-card" style="margin-bottom: 16px;">
            ...
            <div style="padding: 16px; background: var(--bg); border-radius: 8px;">
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 12px;">
                    <span style="font-weight: 600; font-size: 14px; color: var(--text);">${aq.item || '-'}</span>
                    <span style="padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; background: ${statusColor}22; color: ${statusColor};">${statusLabel}</span>
                </div>
            </div>
        </div>
        ...
        <div class="form-card" style="margin-bottom: 16px;">
            ...
            <div style="padding: 16px; background: var(--bg); border-radius: 8px;">
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--primary);"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
                    <div>
                        <div style="font-size: 12px; color: var(--text-muted);">Nº da Compra</div>
                        <div style="font-weight: 600; font-size: 14px; color: var(--text);">${aq.numeroCompra || '-'}</div>
                    </div>
                </div>
                ${aq.numeroNota ? `
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--primary);"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                    <div>
                        <div style="font-size: 12px; color: var(--text-muted);">Nº da Nota</div>
                        <div style="font-weight: 600; font-size: 14px; color: var(--text);">${aq.numeroNota}</div>
                    </div>
                </div>
                ` : ''}
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--primary);"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                    <div>
                        <div style="font-size: 12px; color: var(--text-muted);">Data</div>
                        <div style="font-weight: 600; font-size: 14px; color: var(--text);">${dataFmt}</div>
                    </div>
                </div>
                <div style="display: flex; align-items: center; gap: 12px;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--primary);"><circle cx="12" cy="12" r="10"/><path d="M16 8h-6a2 2 0 1 0 0 4h4a2 2 0 1 1 0 4H8"/><path d="M12 18V6"/></svg>
                    <div>
                        <div style="font-size: 12px; color: var(--text-muted);">Valor</div>
                        <div style="font-weight: 600; font-size: 14px; color: var(--text);">${valorFmt}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="form-card" style="margin-bottom: 16px;">
            <div class="form-card-title">
                ...
                Responsável e Veículo
            </div>
            <div style="padding: 16px; background: var(--bg); border-radius: 8px;">
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--primary);"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                    <div>
                        <div style="font-size: 12px; color: var(--text-muted);">Responsável</div>
                        <div style="font-weight: 600; font-size: 14px; color: var(--text);">${aq.responsavel || '-'}</div>
                    </div>
                </div>
                <div style="display: flex; align-items: center; gap: 12px;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--primary);"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
                    <div>
                        <div style="font-size: 12px; color: var(--text-muted);">Veículo</div>
                        <div style="font-weight: 600; font-size: 14px; color: var(--text);">${aq.veiculo || '-'}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="form-card" style="margin-bottom: 16px;">
            <div class="form-card-title">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 21h18"/><path d="M5 21V7l8-4 8 4v14"/><path d="M17 21v-8.5a1.5 1.5 0 0 0-3 0V21"/><path d="M7 21v-5a1.5 1.5 0 0 0-3 0V21"/></svg>
                Fornecedor
            </div>
            <div style="padding: 16px; background: var(--bg); border-radius: 8px;">
                <div style="display: flex; align-items: center; gap: 12px;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color: var(--primary);"><path d="M3 21h18"/><path d="M5 21V7l8-4 8 4v14"/><path d="M17 21v-8.5a1.5 1.5 0 0 0-3 0V21"/><path d="M7 21v-5a1.5 1.5 0 0 0-3 0V21"/></svg>
                    <div>
                        <div style="font-size: 12px; color: var(--text-muted);">Fornecedor</div>
                        <div style="font-weight: 600; font-size: 14px; color: var(--text);">${aq.fornecedor || '-'}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="form-card" style="margin-bottom: 16px;">
            ...
            <div style="padding: 16px; background: #fff5f5; border-radius: 8px; border: 1px solid #fc8181;">
                <div style="font-size: 14px; color: #c53030;">${aq.motivo}</div>
            </div>
        </div>
        ` : ''}
        ...
        <div class="form-card" style="margin-bottom: 16px;">
            ...
            <div style="padding: 16px; background: var(--bg); border-radius: 8px;">
                <div style="font-size: 14px; color: var(--text);">${aq.obs}</div>
            </div>
        </div>
        ` : ''}

        <button class="btn-submit" id="btn-voltar-historico" style="margin-top: 24px;">
...
function iniciarNovaAquisicaoMobile() {
    ...
    document.getElementById('aq-tem-nota-mobile').checked = false;
    document.getElementById('aq-nota-mobile').value = '';
    document.getElementById('aq-nota-grupo-mobile').classList.add('hidden');
...
function editarAquisicaoMobileSelecionada() {
    ...
    const temNota = !!aq.numeroNota;
    document.getElementById('aq-tem-nota-mobile').checked = temNota;
    document.getElementById('aq-nota-mobile').value = aq.numeroNota || '';
    if (temNota) {
        document.getElementById('aq-nota-grupo-mobile').classList.remove('hidden');
    } else {
        document.getElementById('aq-nota-grupo-mobile').classList.add('hidden');
    }
...
let currentNotDoneStatus = 'nao-concluido';

function abrirMotivoNaoRealizadoMobile(status = 'nao-concluido') {
    const aq = getAquisicaoMobileSelecionada();
    if (!aq) return;
    fecharMenuAquisicaoMobile();
    currentNotDoneStatus = status;

    const titleEl = document.querySelector('#aq-reason-modal h3');
    if (titleEl) {
        titleEl.textContent = status === 'nao-encontrado' ? 'Não encontrado na rua' : 'Motivo do não realizado';
    }

    document.getElementById('aq-not-done-reason').value = aq.motivo || (status === 'nao-encontrado' ? 'Não encontrado na rua' : '');
...
async function salvarNaoRealizadoMobile() {
    const aq = getAquisicaoMobileSelecionada();
    let motivo = document.getElementById('aq-not-done-reason').value.trim();
    if (!aq) return;
    if (!motivo) {
        if (currentNotDoneStatus === 'nao-encontrado') {
            motivo = 'Não encontrado na rua';
        } else {
    ...
    try {
        const res = await fetch(`${serverUrl}/api/aquisicao/${encodeURIComponent(aq.id)}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ status: currentNotDoneStatus, motivo, origem: aq.origem || 'mobile-app' }),
            signal: AbortSignal.timeout(8000)
        });
...
document.getElementById('form-aquisicao')?.addEventListener('submit', async (e) => {
    ...
    const temNota = document.getElementById('aq-tem-nota-mobile').checked;
    const numeroNota = temNota ? document.getElementById('aq-nota-mobile').value.trim() : '';
    ...
    const novaAq = {
        ...
        numeroCompra, numeroNota, item, data, fornecedor, veiculo, 
        ...
        document.getElementById('success-details-aq').innerHTML = `
            ...
            ${novaAq.numeroNota ? `
                <div class="success-line">
                    ...
                    <div><strong>Nº Nota:</strong> ${novaAq.numeroNota}</div>
                </div>
            ` : ''}
            <div class="success-line">
...
let chartGastosVeiculoInstance = null;
let chartGastosSemanaInstance = null;
let rankingPlacasCompleto = [];
let aqPageMobile = 0;
let mostrarTodosVeiculosGastos = false;
const AQ_PER_PAGE_MOBILE = 10;
...
async function carregarDashboard() {
    ...
    try {
        ...
        rankingPlacasCompleto = Object.entries(contagemPlacas).sort((a,b) => b[1] - a[1]);
        const placasOrdenadas = rankingPlacasCompleto.slice(0, 10);
...
function renderizarGraficosDashboard(aquisicoes) {
    // Old dashboard charts are removed
}

function getSemanaDoMes(dateStr) {
...
async function carregarCustos() {
    ...
    try {
        ...
        const aquisicoesValidas = aquisicoes.filter(a => a.status !== 'nao-concluido' && a.status !== 'nao-encontrado');
        const aquisicoesMes = aquisicoesValidas.filter(a => {
            if (!a.data) return false;
            const parts = a.data.split('-');
            return parts.length === 3 && parseInt(parts[0]) === anoAtual && parseInt(parts[1]) === mesAtual;
        });
        
        const totalCost = aquisicoesMes.reduce((sum, a) => sum + (parseFloat(a.valor) || 0) * (parseInt(a.quantidade) || 1), 0);
        
        const elTotal = document.getElementById('custos-total-mes');
        if (elTotal) {
            elTotal.textContent = `R$ ${totalCost.toFixed(2).replace('.', ',')}`;
        }
        
        renderizarGraficosCustos(aquisicoes, aquisicoesValidas, aquisicoesMes);
...
function renderizarGraficosCustos(aquisicoes, aquisicoesValidas, aquisicoesMes) {
    if (typeof Chart === 'undefined') return;
    
    // 1. Onda: Realizadas vs Não Realizadas
    const datas = {};
    aquisicoes.forEach(a => {
        if (!a.data) return;
        ...
        datas[mesAno].total++;
        if (a.status === 'nao-concluido' || a.status === 'Não Concluído' || a.status === 'nao-encontrado') datas[mesAno].fail++;
    });
    ...
    // 2. Gastos por veículo (Barra horizontal)
    const gastosPorVeiculo = {};
    aquisicoesValidas.forEach(a => {
        if (a.veiculo) {
            const custo = (parseFloat(a.valor) || 0) * (parseInt(a.quantidade) || 1);
            gastosPorVeiculo[a.veiculo] = (gastosPorVeiculo[a.veiculo] || 0) + custo;
        }
    });
    const topVeiculosGastos = Object.entries(gastosPorVeiculo)
        .sort((a,b) => b[1] - a[1])
        .slice(0, mostrarTodosVeiculosGastos ? undefined : 5);
        
    const ctxVeic = document.getElementById('chart-mobile-gastos-veiculo');
    if (ctxVeic) {
        if (chartGastosVeiculoInstance) chartGastosVeiculoInstance.destroy();
        chartGastosVeiculoInstance = new Chart(ctxVeic, {
            type: 'bar',
            data: {
                labels: topVeiculosGastos.map(x => x[0].split(' - ')[0]),
                datasets: [{
                    label: 'Gasto (R$)',
                    data: topVeiculosGastos.map(x => x[1]),
    ...
    // 3. Gastos por semana (Linha)
    const semanas = { 'Semana 1': 0, 'Semana 2': 0, 'Semana 3': 0, 'Semana 4': 0 };
    aquisicoesMes.forEach(a => {
        const sem = getSemanaDoMes(a.data);
        const custo = (parseFloat(a.valor) || 0) * (parseInt(a.quantidade) || 1);
        if (semanas[sem] !== undefined) {
            semanas[sem] += custo;
        }
    });
    
    const ctxSemana = document.getElementById('chart-mobile-gastos-semana');
    if (ctxSemana) {
        if (chartGastosSemanaInstance) chartGastosSemanaInstance.destroy();
        chartGastosSemanaInstance = new Chart(ctxSemana, {
            type: 'line',
            data: {
                labels: ['Semana 1', 'Semana 2', 'Semana 3', 'Semana 4'],
                datasets: [{
                    label: 'Gasto por Semana (R$)',
                    data: [semanas['Semana 1'], semanas['Semana 2'], semanas['Semana 3'], semanas['Semana 4']],
                    borderColor: '#105444',
                    backgroundColor: 'rgba(16,84,68,0.1)',
                    tension: 0.3,
                    fill: true
                }]
            },
...
document.addEventListener('DOMContentLoaded', () => {
    // Toggle para nota fiscal
    const chkNotaMobile = document.getElementById('aq-tem-nota-mobile');
    if (chkNotaMobile) {
        chkNotaMobile.addEventListener('change', () => {
            const grp = document.getElementById('aq-nota-grupo-mobile');
            if (grp) {
                if (chkNotaMobile.checked) {
    ...
    // Toggle para mostrar todos os veículos no gráfico de gastos
    document.getElementById('btn-ver-todos-gastos')?.addEventListener('click', () => {
        mostrarTodosVeiculosGastos = !mostrarTodosVeiculosGastos;
        document.getElementById('btn-ver-todos-gastos').textContent = mostrarTodosVeiculosGastos ? 'Ver top 5' : 'Ver todos';
        carregarDashboard(); // Recarregar o dashboard para atualizar o gráfico
    });

    // Modal de ranking completo
    document.getElementById('btn-ver-todas-placas')?.addEventListener('click', () => {
        const listEl = document.getElementById('lista-ranking-completa');
        if (listEl) {
            if (rankingPlacasCompleto.length > 0) {
                listEl.innerHTML = rankingPlacasCompleto.map((p, index) => `
                    <div class="dash-list-item" style="padding: 10px 0; border-bottom: 1px solid var(--border); display:flex; justify-content:space-between;">
                        <div style="display:flex; align-items:center; gap:12px;">
                            <span style="font-weight:800; color:var(--primary); font-size:15px; min-width:24px;">#${index+1}</span>
                            <span style="font-weight:600; font-size:14px;">${p[0]}</span>
                        </div>
                        <span style="font-weight:700; font-size:14px;">${p[1]} aq.</span>
                    </div>
                `).join('');
            } else {
                listEl.innerHTML = '<div class="empty-state">Nenhuma aquisição.</div>';
            }
        }
        document.getElementById('modal-ranking-placas')?.classList.remove('hidden');
    });

    document.getElementById('btn-fechar-ranking-placas')?.addEventListener('click', () => {
        document.getElementById('modal-ranking-placas')?.classList.add('hidden');
    });
    ...
    document.getElementById('drawer-custos')?.addEventListener('click', () => {
        fecharDrawer();
        irParaTela('screen-custos');
        carregarCustos();
    });
    
    document.getElementById('btn-menu-custos')?.addEventListener('click', abrirDrawer);
    
    document.getElementById('btn-config-custos')?.addEventListener('click', () => {
        const input = document.getElementById('config-server-url');
        if (input) input.value = serverUrl;
        fecharDrawer();
        mostrarTela('screen-config');
