...
// ============================================================
// DASHBOARD LOGIC E HISTORICO DE AQUISIÃ‡ÃƒO
// ============================================================
...
async function carregarDashboard() {
    ...
    try {
        ...
        const aqConcluidas = aquisicoes.filter(a => a.status === 'concluido' || a.status === 'ConcluÃ­do').length;
        const aqNaoRealizadas = aquisicoes.filter(a => a.status === 'nao-concluido' || a.status === 'NÃ£o ConcluÃ­do').length;
        ...
        if (listaLembretesEl) {
            ...
            if (lembretesProximos.length > 0) {
                ...
                listaLembretesEl.innerHTML = '<div class="empty-state">Nenhum lembrete para os prÃ³ximos 3 dias.</div>';
            }
        }
        ...
        if (rankingEl) {
            if (placasOrdenadas.length > 0) {
                ...
                rankingEl.innerHTML = '<div class="empty-state">Nenhuma aquisiÃ§Ã£o.</div>';
            }
        }
...
function renderizarGraficosDashboard(aquisicoes) {
    ...
    aquisicoes.forEach(a => {
        ...
        datas[mesAno].total++;
        if (a.status === 'nao-concluido' || a.status === 'NÃ£o ConcluÃ­do') datas[mesAno].fail++;
    });
