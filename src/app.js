...
const DEFAULT_SERVER = 'http://147.93.12.196:7011';
...
// Responsáveis por busca (quem busca itens)
const responsaveisBusca = [
    "MARCELINO SANTOS",
    "MARCELO DE ABREU MOTTA",
    "MARCELO JORGE DA SILVA FARIAS",
    "ROGERIO LOPES DE SOUSA",
    "RAFAEL SAINT'ANNA FARIAS",
    "RAFAEL DE JESUS",
    "FERNANDO ROSA",
    "MIQUEIAS MARINHO SILVA",
    "ADRIEL SIQUEIRA ACACIO",
    "ANDRE HENRIQUE ANDRADE DE SOUZA"
];
...
function inicializar() {
    ...
    // Pré-preencher campo de URL com IP do servidor
    const urlInput = document.getElementById('config-server-url');
    if (urlInput) {
        urlInput.placeholder = DEFAULT_SERVER;
        if (!serverUrl) urlInput.value = DEFAULT_SERVER;
    }
...
// AQUISIÇÕES (MOBILE)
// ============================================================
// Filtro dinâmico de fornecedores conforme digitação
function filtrarFornecedores(valor) {
    const dlForn = document.getElementById('aq-forn-list-mobile');
    if (!dlForn || typeof fornecedores === 'undefined') return;
    
    const termo = valor.toLowerCase().trim();
    const options = fornecedores.filter(f => 
        f.razaoSocial.toLowerCase().includes(termo) || 
        (f.cnpj && f.cnpj.toLowerCase().includes(termo))
    );
    
    dlForn.innerHTML = options.map(f => 
        `<option value="${f.razaoSocial}">`
    ).join('');
}
...
// ============================================================
// OFFLINE SUPPORT - FETCH WITH QUEUE
// ============================================================
let offlineQueue = [];
...
async function fetchWithOfflineSupport(url, options) {
    if (isOnline) {
        try {
            ...
            return { ok: response.ok, offline: false, data: await response.json() };
        } catch (error) {
...
// ============================================================
// FINANCEIRO - MONITORAMENTO DE GASTOS
// ============================================================
let finChartResumo = null;
let finChartMensal = null;

function initFinanceiro() {
    document.getElementById('btn-menu-financeiro')?.addEventListener('click', abrirDrawer);
    document.getElementById('drawer-financeiro')?.addEventListener('click', () => {
        fecharDrawer();
        irParaTela('screen-financeiro');
        carregarFinanceiro();
    });
    document.getElementById('btn-refresh-financeiro')?.addEventListener('click', carregarFinanceiro);
    document.getElementById('btn-fechar-fin-modal')?.addEventListener('click', () => {
        document.getElementById('fin-modal-veiculos').classList.add('hidden');
    });
    document.getElementById('fin-ver-todos-veiculos')?.addEventListener('click', abrirModalTodosVeiculos);
    document.getElementById('fin-ver-ranking-completo')?.addEventListener('click', () => {
        document.querySelector('.fin-tab[data-fintab="fin-veiculos"]')?.click();
    });
    document.getElementById('fin-filtro-veiculo')?.addEventListener('input', (e) => {
        renderFinanceiroVeiculos(e.target.value);
    });
...
async function carregarFinanceiro() {
    ...
    try {
        ...
        const aquisicoes = await res.json();
        renderFinanceiroResumo(aquisicoes);
        renderFinanceiroVeiculos('');
        renderFinanceiroTop(aquisicoes);
        renderFinanceiroFornecedores(aquisicoes);
        renderFinanceiroResponsaveis(aquisicoes);
        renderFinanceiroMensal(aquisicoes);
    } catch (err) {
        console.error('Erro ao carregar financeiro:', err);
    }
}

function renderFinanceiroResumo(aquisicoes) {
    const comValor = aquisicoes.filter(a => a.valor && a.status !== 'nao-concluido');
    const totalGasto = comValor.reduce((s, a) => s + parseFloat(a.valor || 0), 0);
    const totalItens = comValor.length;
    const veiculosSet = new Set(comValor.filter(a => a.veiculo).map(a => a.veiculo));
    const fornecedoresSet = new Set(comValor.filter(a => a.fornecedor).map(a => a.fornecedor));

    document.getElementById('fin-resumo-total').textContent = `R$ ${totalGasto.toFixed(2).replace('.', ',')}`;
    document.getElementById('fin-resumo-itens').textContent = totalItens;
    document.getElementById('fin-resumo-veiculos').textContent = veiculosSet.size;
    document.getElementById('fin-resumo-fornecedores').textContent = fornecedoresSet.size;

    // Gráfico resumo - gasto por veículo (top 5)
    const gastoPorVeiculo = {};
    comValor.forEach(a => {
        if (a.veiculo && a.valor) {
            gastoPorVeiculo[a.veiculo] = (gastoPorVeiculo[a.veiculo] || 0) + parseFloat(a.valor);
        }
    });
    const top5 = Object.entries(gastoPorVeiculo).sort((a,b) => b[1] - a[1]).slice(0, 5);
    
    const ctx = document.getElementById('chart-fin-resumo');
    if (ctx) {
        if (finChartResumo) finChartResumo.destroy();
        if (top5.length > 0) {
            finChartResumo = new Chart(ctx, {
                type: 'doughnut',
                data: {
                    labels: top5.map(t => t[0]),
                    datasets: [{
                        data: top5.map(t => t[1]),
                        backgroundColor: ['#105444', '#2d6a4f', '#40916c', '#52b788', '#95d5b2'],
                        borderWidth: 0
                    }]
                },
    ...
    // Últimas aquisições
    const lista = document.getElementById('fin-resumo-lista');
    const recentes = [...aquisicoes].sort((a,b) => new Date(b.data || 0) - new Date(a.data || 0)).slice(0, 5);
    if (recentes.length === 0) {
        lista.innerHTML = '<div class="empty-state">Nenhuma aquisição registrada.</div>';
    } else {
        lista.innerHTML = recentes.map(a => {
            const valor = a.valor ? `R$ ${parseFloat(a.valor).toFixed(2).replace('.', ',')}` : '-';
            return `<div class="dash-list-item"><div><div style="font-weight:600;font-size:14px;">${a.item || '-'}</div><div style="font-size:12px;color:var(--text-secondary);">${a.veiculo || '-'} · ${a.fornecedor || '-'}</div></div><div style="font-weight:700;color:var(--primary);">${valor}</div></div>`;
        }).join('');
    }
}

function renderFinanceiroVeiculos(filtro) {
    const lista = document.getElementById('fin-veiculos-lista');
    if (!lista) return;
    
    fetch(`${serverUrl}/api/aquisicoes`, { signal: AbortSignal.timeout(15000) }).then(r => r.json()).then(aquisicoes => {
        const comValor = aquisicoes.filter(a => a.valor && a.veiculo && a.status !== 'nao-concluido');
        const gastoPorVeiculo = {};
        comValor.forEach(a => {
            gastoPorVeiculo[a.veiculo] = (gastoPorVeiculo[a.veiculo] || 0) + parseFloat(a.valor);
        });
        let entries = Object.entries(gastoPorVeiculo).sort((a,b) => b[1] - a[1]);
        if (filtro) entries = entries.filter(e => e[0].toLowerCase().includes(filtro.toLowerCase()));
        
        if (entries.length === 0) {
            lista.innerHTML = '<div class="empty-state">Nenhum gasto por veículo.</div>';
        } else {
            lista.innerHTML = entries.map((e, i) => {
                const cor = i < 3 ? ['#ffd700','#c0c0c0','#cd7f32'][i] : 'transparent';
                return `<div class="dash-list-item"><div style="display:flex;align-items:center;gap:10px;"><span style="font-weight:800;color:var(--primary);font-size:16px;width:24px;">#${i+1}</span><span style="font-weight:600;">${e[0]}</span></div><span style="font-weight:700;">R$ ${e[1].toFixed(2).replace('.', ',')}</span></div>`;
            }).join('');
        }
    }).catch(() => {
        lista.innerHTML = '<div class="empty-state">Erro ao carregar.</div>';
    });
}

function renderFinanceiroTop(aquisicoes) {
    const lista = document.getElementById('fin-top-lista');
    if (!lista) return;
    ...
    const top = Object.entries(gastoPorVeiculo).sort((a,b) => b[1] - a[1]).slice(0, 10);
    
    if (top.length === 0) {
        lista.innerHTML = '<div class="empty-state">Nenhum dado disponível.</div>';
    } else {
        lista.innerHTML = top.map((e, i) => {
            const medalha = ['🥇','🥈','🥉'][i] || '';
            return `<div class="dash-list-item"><div style="display:flex;align-items:center;gap:8px;"><span style="font-size:18px;">${medalha}</span><span style="font-weight:600;">${e[0]}</span></div><span style="font-weight:700;color:var(--primary);">R$ ${e[1].toFixed(2).replace('.', ',')}</span></div>`;
        }).join('');
    }
}

function renderFinanceiroFornecedores(aquisicoes) {
    const lista = document.getElementById('fin-fornecedores-lista');
    if (!lista) return;
    
    const comValor = aquisicoes.filter(a => a.valor && a.fornecedor && a.status !== 'nao-concluido');
    const gastoPorForn = {};
    comValor.forEach(a => {
        gastoPorForn[a.fornecedor] = (gastoPorForn[a.fornecedor] || 0) + parseFloat(a.valor);
    });
    const entries = Object.entries(gastoPorForn).sort((a,b) => b[1] - a[1]);
    
    if (entries.length === 0) {
        lista.innerHTML = '<div class="empty-state">Nenhum gasto por fornecedor.</div>';
    } else {
        lista.innerHTML = entries.map(e => `<div class="dash-list-item"><span style="font-weight:600;">${e[0]}</span><span style="font-weight:700;color:var(--primary);">R$ ${e[1].toFixed(2).replace('.', ',')}</span></div>`).join('');
    }
}

function renderFinanceiroResponsaveis(aquisicoes) {
    const lista = document.getElementById('fin-responsaveis-lista');
    if (!lista) return;
    
    const comValor = aquisicoes.filter(a => a.valor && a.responsavel && a.status !== 'nao-concluido');
    const gastoPorResp = {};
    comValor.forEach(a => {
        gastoPorResp[a.responsavel] = (gastoPorResp[a.responsavel] || 0) + parseFloat(a.valor);
    });
    const entries = Object.entries(gastoPorResp).sort((a,b) => b[1] - a[1]);
    
    if (entries.length === 0) {
        lista.innerHTML = '<div class="empty-state">Nenhum gasto por responsável.</div>';
    } else {
        lista.innerHTML = entries.map(e => `<div class="dash-list-item"><span style="font-weight:600;">${e[0]}</span><span style="font-weight:700;color:var(--primary);">R$ ${e[1].toFixed(2).replace('.', ',')}</span></div>`).join('');
    }
}

function renderFinanceiroMensal(aquisicoes) {
    const comValor = aquisicoes.filter(a => a.valor && a.data && a.status !== 'nao-concluido');
    const gastoPorMes = {};
    comValor.forEach(a => {
        const mes = a.data.substring(0, 7);
        gastoPorMes[mes] = (gastoPorMes[mes] || 0) + parseFloat(a.valor);
    });
    const labels = Object.keys(gastoPorMes).sort();
    const data = labels.map(l => gastoPorMes[l]);
    const formatedLabels = labels.map(l => { const p = l.split('-'); return p[1] + '/' + p[0]; });

    const ctx = document.getElementById('chart-fin-mensal');
    if (ctx) {
        if (finChartMensal) finChartMensal.destroy();
        if (labels.length > 0) {
            finChartMensal = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: formatedLabels,
                    datasets: [{
    ...
    const tabela = document.getElementById('fin-mensal-tabela');
    if (tabela) {
        if (labels.length === 0) {
            tabela.innerHTML = '<div class="empty-state">Nenhum dado mensal.</div>';
        } else {
            tabela.innerHTML = labels.map((l, i) => {
                return `<div class="dash-list-item"><span style="font-weight:600;">${formatedLabels[i]}</span><span style="font-weight:700;color:var(--primary);">R$ ${data[i].toFixed(2).replace('.', ',')}</span></div>`;
            }).reverse().join('');
        }
    }
}

function abrirModalTodosVeiculos() {
    fetch(`${serverUrl}/api/aquisicoes`, { signal: AbortSignal.timeout(15000) }).then(r => r.json()).then(aquisicoes => {
        const comValor = aquisicoes.filter(a => a.valor && a.veiculo && a.status !== 'nao-concluido');
        const gastoPorVeiculo = {};
        comValor.forEach(a => {
        ...
        const entries = Object.entries(gastoPorVeiculo).sort((a,b) => b[1] - a[1]);
        const lista = document.getElementById('fin-modal-veiculos-lista');
        if (entries.length === 0) {
            lista.innerHTML = '<div class="empty-state">Nenhum gasto registrado.</div>';
        } else {
            lista.innerHTML = entries.map((e, i) => `<div class="dash-list-item"><span style="font-weight:600;">${i+1}. ${e[0]}</span><span style="font-weight:700;color:var(--primary);">R$ ${e[1].toFixed(2).replace('.', ',')}</span></div>`).join('');
        }
        document.getElementById('fin-modal-veiculos').classList.remove('hidden');
    }).catch(() => {
        document.getElementById('fin-modal-veiculos-lista').innerHTML = '<div class="empty-state">Erro ao carregar.</div>';
        document.getElementById('fin-modal-veiculos').classList.remove('hidden');
    });
}
